...
Name: Abeer Ismail
Date: 7/28/2023
Purpose: To create a hashmap using a predefined hash function to handle all inputs which will be inputted into a huffman encoder
Notes: functions are tested in tests.cpp
...
void hashmap::put(int key, int value) {
    ...
    key_val_pair* front = buckets[bucket];
    key_val_pair* prev = nullptr;

    //creates a doubly linked list to input the key
    while (front != nullptr){
        if (front->key == key){
            front->value = value;
            return;
    ...
    //This handles when there isnt already a list there
...
int hashmap::get(int key) const {
    ...
    throw runtime_error("Key was not found in the Hashmap");
}
...
bool hashmap::containsKey(int key) {
    ...
    //manually traversing the linked list to find a key
    while (front != nullptr){
        if (front->key == key){
            return true;
        }
...
vector<int> hashmap::keys() const {
    vector<int> allKeys; //Since each bucket can hold multiple keys its inputted into a vector

    for (int i = 0; i < nBuckets; i++){
        key_val_pair* front = buckets[i];
        while (front != nullptr){
            allKeys.push_back(front->key);
            front = front->next;
        }
    }
    return allKeys;
